...
// Version: v3.0.0
// Inspired by CFnew / byJoey — Enhanced with smart engine, live map, user profiles
let 连接 = null;
try { 连接 = (await import("cloudflare:sockets")).connect; } catch {}
...
let 认证令牌 = ''; // از طریق پنل به‌عنوان متغیر u تزریق می‌شود — هیچ کلیدی در مخزن ذخیره نشود
let 回退地址 = '';
let 代理5配置 = '';
let 自定义优选地址列表 = [];
let 自定义优选域名列表 = [];
...
export default {
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      启用木马 = 获取配置开关值('et', true, 本地值734.et);
      ...
      if (网址698.pathname.includes('/api/update')) {
        ...
        // NEXUS: User profile endpoint
        if (网址698.pathname.includes('/api/user-config')) {
          const uid = 网址698.searchParams.get('uid') || '';
          const ukey = uid ? 'u_' + uid : 'u_default';
          if (请求735.method === 'GET') {
            try {
              const raw = 键值存储 ? await 键值存储.get(ukey) : null;
              const ucfg = raw ? JSON.parse(raw) : {};
              return new Response(JSON.stringify({ ok: true, ucfg }), { headers: { 'Content-Type': 'application/json' } });
            } catch (e) {
              return new Response(JSON.stringify({ ok: true, ucfg: {} }), { headers: { 'Content-Type': 'application/json' } });
            }
          }
          if (请求735.method === 'POST') {
            try {
              const body = await 请求735.json();
              if (键值存储) await 键值存储.put(ukey, JSON.stringify(body));
              return new Response(JSON.stringify({ ok: true, ucfg: body }), { headers: { 'Content-Type': 'application/json' } });
            } catch (e) {
  ...
  if (输入) {
    ...
    输入.addEventListener('keypress', function (事件值) {
      if (事件值.key === 'Enter') {
        ...
        if (值路径 && 网址698.pathname.toLowerCase().includes(`/${值路径}`)) {
...
async function 处理订阅值(请求241, 用户240 = null) {
  ...
  const 本地值235 = {
    zh: {
      title: 'NEXUS',
      subtitle: 'The Gateway to Tomorrow — Smart • Fast • Secure',
      selectClient: '[ 选择客户端 ]',
      systemStatus: '[ 系统状态 ]',
    ...
    fa: {
      title: 'NEXUS',
      subtitle: 'دروازه به فردا — هوشمند • سریع • امن',
  ...
  const 值页面 = `<!DOCTYPE html>
    ...
    <body>
        ...
        <div class="matrix-code-rain" id="matrixCodeRain"></div>
            <header class="cp-bar">
                <div class="cp-brand"><span class="cp-mk"></span><span><b>NEXUS</b><small>${翻译值.terminal}</small></span></div>
        ...
        <div class="container">
            ...
            <div class="card">
                ...
                <div style="text-align: center; margin: 20px 0;">
                        <a href="https://github.com/miladjahani/miliconfigpro-v1" target="_blank" style="color: #c8f135; text-decoration: none; margin: 0 20px; font-size: 1.2rem; ">${翻译值.githubProject}</a>
