...
import { generateDockerCompose, generateNginxConf, generateEnvFile, generateDeployScript } from '../lib/vps-deploy'
...
export default function DeployWizard() {
  ...
  const [method, setMethod] = useState<'workers' | 'pages' | 'vps'>('workers')
  ...
  const handleDeploy = async () => {
    ...
    // VPS mode: generate Docker files and download as ZIP
    if (method === 'vps') {
      setDeployLogs(['در حال تولید فایل‌های Docker...'])
      try {
        const vpsPort = '8080'
        const zip = new JSZip()
        zip.file('docker-compose.yml', generateDockerCompose({ name, uuid, adminPassword: adminPassword || uuid, domain: '', port: vpsPort }))
        zip.file('nginx.conf', generateNginxConf({ name, uuid, adminPassword: adminPassword || uuid, domain: '', port: vpsPort }))
        zip.file('.env', generateEnvFile({ name, uuid, adminPassword: adminPassword || uuid, domain: '', port: vpsPort }))
        zip.file('deploy.sh', generateDeployScript({ name, uuid, adminPassword: adminPassword || uuid, domain: '', port: vpsPort }))
        zip.file('README.md', `# ${name} — StanNG v2 Docker Deployment\n\n## Quick Start\n\n1. Upload this ZIP to your VPS\n2. Extract: \`unzip ${name}-stanng.zip\`\n3. Run: \`bash deploy.sh\`\n\n## Manual Setup\n\n\`\`\`bash\nmkdir -p stanng-src stanng-data certbot/conf certbot/www\ngit clone --depth 1 https://github.com/youdidking/stanngv2.git stanng-src\ndocker compose up -d\n\`\`\`\n\n## Panel Access\n\n- URL: http://YOUR_SERVER_IP:8080/login\n- Password: \`${adminPassword || uuid}\`\n\n## HTTPS Setup\n\n1. Point your domain DNS to the server\n2. Edit \`.env\` → set \`PUBLIC_DOMAIN=your.domain.com\`\n3. Run certbot:\n\n\`\`\`bash\ndocker compose --profile certonly run --rm certbot certonly --webroot -w /var/www/certbot -d your.domain.com\n\`\`\`\n\n4. Uncomment HTTPS block in \`nginx.conf\`\n5. Restart: \`docker compose restart nginx\`\n\n---\nGenerated by miliconfigpro panel\n`)  
        ...
        setDeploying(false)
        setDeployResult({ success: true, message: 'فایل‌های Docker تولید و دانلود شدند!', url: `http://YOUR_SERVER_IP:${vpsPort}`, panelUrl: `http://YOUR_SERVER_IP:${vpsPort}/login` })
      } catch (err) {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{method === 'vps' ? 'فایل‌های Docker برای استقرار StanNG روی VPS تولید و دانلود می‌شوند' : 'ورکر به‌صورت خودکار از مخزن دانلود و روی کلودفلر مستقر می‌شود — نیازی به کدنویسی نیست'}</p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-4 flex items-center gap-3 border-brand-500/20">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">{method === 'vps' ? 'فایل‌های docker-compose.yml، nginx.conf، .env و deploy.sh تولید و به‌صورت ZIP دانلود می‌شوند.' : 'کد ورکر از مخزن GitHub بارگذاری می‌شود، KV ساخته می‌شود، bindings تنظیم می‌شود و ورکر روی edge مستقر می‌گردد.'}</p>
        </div>
      </div>
            ...
            <div>
              ...
              {method === 'vps' ? (
                <select value={workerSource} onChange={(e) => setWorkerSource(e.target.value)} className="input-field">
                  <option value="stanngv2">StanNG v2 — پنل VLESS با xray-core (Docker + VPS)</option>
                </select>
              ) : (
                <select value={workerSource} onChange={(e) => setWorkerSource(e.target.value)} className="input-field">
            ...
            {!deployResult && (
              <div className="flex flex-col items-center gap-4 py-6">
                ...
                >
                  ...
                  {deploying ? (method === 'vps' ? 'در حال تولید فایل‌ها...' : 'در حال استقرار...') : (method === 'vps' ? 'دانلود فایل‌های Docker' : 'استقرار ورکر')}
                </button>
              ...
              <div className="animate-slide-up">
                {deployResult.success ? (
                  ...
                  <div className="text-center py-6">
                    ...
                    {method === 'vps' && deployResult.success && (
